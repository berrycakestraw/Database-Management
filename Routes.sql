...
select * from TourPackages where  
price > (select avg(price) from TourPackages)

-- customers from asia continent
select * from Customers where CountryID in 
( select CountryID from Countries where Continent = 'Asia');

select PackageName as 'Package Name', CONCAT(Price, '$') as newPrice, cast(Price as varchar(100)) + '$' as 'Price in Dollar'  from TourPackages 
where  price > (select avg(price)+$ from TourPackages)

--find all packages that are more expensive than the cheapest package
select packagename from TourPackages where
price > (select min(price) from TourPackages)

--find customers who have booked at least one package
select * from Customers where CustomerID in (select CustomerID from Bookings)

--find customers who have never booked any package
select * from Customers where CustomerID NOT IN (select CustomerID from Bookings)

--find tour packages that have been booked by customer 'aarav sharma'
select * from TourPackages
where PackageID in (select PackageID from Bookings
    where CustomerID in (select CustomerID from Customers where FullName = 'Aarav Sharma'
    )
);